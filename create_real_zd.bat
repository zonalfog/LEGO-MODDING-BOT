...
echo .zip to REAL .zd Converter
echo ==========================
echo.

echo Select a .zip file to convert to REAL .zd format:
echo.
...
echo Creating REAL .zd file (not just renamed zip)...
echo This creates a proprietary format that cannot be extracted as normal zip!
echo.

python create_real_zd.py create "%input_file%" "%output_file%"

if %errorlevel% equ 0 (
    echo.
    echo ✅ Successfully created REAL .zd file: %output_file%
    echo.
    echo Note: This .zd file uses custom encryption and cannot be extracted
    echo by simply renaming to .zip. Use the special extractor!
) else (
    echo.
    echo ❌ Error during .zd creation
    pause
    exit /b 1
)
